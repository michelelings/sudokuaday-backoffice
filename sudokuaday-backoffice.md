...
|-----------|-------------|
| **M0** | Repo scaffold (Vite React TS), auth gate, empty shell with nav; hosting sends **noindex** headers + `robots.txt` Disallow (§7.1). |
| **M1** | Parity snapshot from **paired HTML paths** (`config.locales.json` + root vs `{locale}/`); first reports: missing/extra files; optional title/description diff. |
| **M2** | Dashboard + drill-down + export; ingest script reads **sudokuaday GitHub** (clone or API) + committed sitemap. |
| **M3** | Structural/slug parity if applicable; freshness rules; historical snapshots (optional). |
| **M4** | GA4 connector + one “traffic by locale/path” view. |
| **M5** | GSC connector + landing/query views aligned with site structure. |
| **M6** | Ahrefs *or* Semrush connector + SEO summary cards. |
