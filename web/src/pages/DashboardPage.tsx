  ...
  return (
    <div className="space-y-8">
      ...
      <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
          <dt className="text-xs font-medium uppercase tracking-wide text-slate-500">Open issues</dt>
          <dd className="mt-1 text-2xl font-semibold tabular-nums">{totalIssues}</dd>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {data.nonDefaultLocales.map((loc) => {
                const s = data.summary[loc] ?? { missing: 0, extra: 0 }
                return (
